...
exports.getMonthlyPlan = catchAsync(async (req, res, next) => {
  ...
  const plan = await Tour.aggregate([
    ...
    {
      $match: {
        startDates: { $gte: new Date(`${year}-01-01`), $lt: new Date(`${year + 1}-01-01`) },
      },
    },
    {
      $group: {
...
exports.getToursWithin = catchAsync(async (req, res, next) => {
  ...
  if (!lat || !lng) next(new AppError("Please provide latitude and langitude in the format lat,lng.", 400));
